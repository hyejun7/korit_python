...
#chr()
#chr(i)는 유니코드를 넣으면 해당 문자로 리턴을 하는 향수
print(chr(97)) #a
print(chr(44032)) #가
...
#divmod()
#2개의 숫자 a, b를 입력 받고 그리고 a를 b로 나눈 몫과 나머지를
#튜플 형태로 리턴
print(divmod(7, 3))
...
#fillter()
#첫 번쨰 인수로 함수, 두 번째 인수로 반복 가능한 데이터
#그리고 반복가능한 데이터의 요소 순서대로 함수를 호출 했을 때
#리턴값이 참인것만 묶어서 리턴
def positive(x):
    return x > 0
...
num_list = [1, 2, 15, 23, 18, 55, 17, 46]
print(max(num_list))
...
#open()
#open(파일이름, 모드)은 파일 이름과 모드를 임력받아
#파일 객체를 리턴하는 함수
# w 쓰기 모드
# r 읽기 모드
# a 추가 모드
